...
<body>
						...
						<div class="datatable-scroll-wrap">
							<table class="table table-hover table-striped dataTable" id="DataTables_Table_SMA" role="grid">
								...
								<tbody>
									...
									if (!empty($dbstaf)) { ?><!--// Jika data siswa tidak sama dengan kosong, artinya jika data siswa ada-->
										<?php foreach ($dbstaf as $item) { ?>
											<tr class='data odd' role='row'>
												...
												<td>
													<button class='btn btn-danger' onclick='deleteDataStaf(<?php echo $item->id; ?>)'>Hapus</button>
												<!--	<button class="btn btn-primary" onclick="editDataStaf(<php echo $item->id; ?>)">Edit</button>-->
												</td>
											</tr>
										<?php } ?>
									...
									<?php
									$totalData = count($dbstaf); // Menghitung jumlah data
									if (!empty($dbstaf)) {
										$index = 1;
	...
	<script>
		...
		function editDataStaf(id) {
            window.location.href = "<?php echo base_url('Admin_dashboard/editDataStaf/'); ?>" + id;
        }
	</script>
